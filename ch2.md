...
console.log( baz.a ); // 3
 ```
Но всегда есть исключения из правил. При явном связывании мы может передать в функции apply, call или bind первым аргументом вместо объекта null и таким образом будет применено связывание по умолчанию. Это часто бывает полезно, но также может вести к трудноотлавливаемым ошибкам. Чтобы избежать подобных ошибок вместо null можно передавать специальный пустой объект, который будет предотвращать мутацию глобального объекта.
```js
function foo(a,b) {
	console.log( "a:" + a + ", b:" + b );
}
...
bar( 3 ); // a:2, b:3
```
Еще один случай это косвенный вызов. Что вернет последняя строчка?
```js
function foo() {
	console.log( this.a );
}
...
Хотя было сказано, что связываение контекста не имеет никакого отношения к лексической области видимости, это изменилось с появлением стрелочных функций в ES6.
Стрелочная функция использует лексическую область видимости для связывания контекста. Она заимствует контекст у включающей ее функции.
```js
function foo() {
...
```
До ES6 многие разработчики проделывали тоже самое с обычными функциями таким образом:
```js
function foo() {
...
foo.call( obj ); // 2
```
И хотя такой прием кажется безобидным, чаще всего им пользуется именно из-за непонимания механизма работы связывания в JS. Чтобы писать более чистый код, нужно либо полностью отказаться от использования стандартного связывания и использовать self и стрелочные функции, либо научиться управлять им.
Конечно программа целиком может содержать в себе разные техники, но внутри одной функции лучше придерживаться одного стиля.

Еще раз приоритет связывания:

1. Связывание через вызов с new
2. Явное и жесткое связывание с использованием call, apply и bind
3. Неявное связывание путем вклыдвания функции в свойство объекта
4. Обычный вызов функции и связывание по умолчанию